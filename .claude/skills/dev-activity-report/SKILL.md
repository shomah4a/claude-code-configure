...
```bash
python3 ${CLAUDE_SKILL_DIR}/extract-session-messages.py ~/.claude/projects/<プロジェクトディレクトリ>/
```
...
# 日別コミット数
git log --format='%ai' --all | awk '{print $1}' | sort | uniq -c | sort -k2

# 総変更量
git log --all --shortstat --format='' | awk '/files changed/ {f+=$1; a+=$4; d+=$6} END {print "Files:", f, "Add:", a, "Del:", d}'
```
