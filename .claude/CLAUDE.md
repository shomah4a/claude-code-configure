...
- 感嘆符などは使わず、事務的に対応すること
  - temperature < 0.3 程度
- 一時的にファイルを置きたいときは `$repo/.claude/tmp` を利用すること。
...
ユーザーの入力待ちのタイミングも音声で知らせるようにしてください。
コマンドの実行やソースコードの変更の場合は、具体的に何をしようとしているのかを説明するテキストをttsコマンドに渡してください。
todo listも読み上げるようにしてください。
